...
class TestRealPDFProcessing:
    ...
    def setup_method(self):
        ...
        self.test_data_dir = Path(__file__).parent.parent.parent / 'training_data' / 'pdf_csv_pairs'
    
    @pytest.mark.parametrize("pdf_name,expected_min_fields", [
        ("W-4R_parsed.pdf", 8),
    ...
    def test_life_1528q_radio_button_detection(self):
        ...
        for expected_group in expected_groups:
            assert expected_group in group_names, f"RadioGroup {expected_group} not found"
    ...
    def test_csv_export_integration(self):
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            expected_columns = [
                'ID', 'Created at', 'Updated at', 'Label', 'Description', 
                'Form ID', 'Order', 'Api name', 'UUID', 'Type'
            ]
    ...
    def test_field_name_consistency(self):
        ...
        radio_button_names = [
            ...
            if name and not name.endswith('--group') and any(
                rg.replace('--group', '') in name for rg in radio_group_names
            )
        ]
        ...
        bem_structured_names = [
            name for name in field_names
            if '_' in name and not name.endswith('--group')
        ]
    ...
    def test_parent_child_relationships(self):
        ...
        for button in radio_buttons:
            ...
            parent_found = any(
                rg.get('ID') == parent_id for rg in radio_groups
            )
            assert parent_found, f"RadioButton {button.get('Api name')} has invalid parent ID {parent_id}"
        
        # Every RadioGroup should have at least one child
        for group in radio_groups:
            group_id = group.get('ID')
            children = [
                b for b in radio_buttons 
                if b.get('Parent ID') == group_id
            ]
            assert len(children) > 0, f"RadioGroup {group.get('Api name')} has no children"
