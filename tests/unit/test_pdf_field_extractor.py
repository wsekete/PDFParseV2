...
class TestPDFFieldExtractor:
    ...
    def test_field_type_detection(self):
        ...
        assert self.extractor._infer_bem_category('test--group', field_data) == 'general-information'
        
        # Test TextField detection
        assert self.extractor._generate_field_label({'name': 'first_name', 'type': 'TextField'}) == 'Name'
        assert self.extractor._generate_field_label({'name': 'last_name', 'type': 'TextField'}) == 'Name'
    
    def test_radiogroup_label_generation(self):
        """Test RadioGroup label generation."""
        test_cases = [
            ({'name': 'address-change--group', 'type': 'RadioGroup'}, 'Address Change Options'),
            ({'name': 'dividend--group', 'type': 'RadioGroup'}, 'Future Dividend Application'),
    ...
    def test_full_extraction_workflow(self):
        ...
        for button in radio_buttons:
            ...
            if parent_id:
                ...
                assert parent_found, f"RadioButton {button.get('Api name')} has invalid parent ID"
