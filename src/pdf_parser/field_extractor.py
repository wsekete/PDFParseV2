...
class PDFFieldExtractor:
    ...
    def extract_fields(
        self, pdf_path: str, context_radius: int = 50, output_format: str = "csv"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if pdf_file.stat().st_size > 100 * 1024 * 1024:  # 100MB limit
                logger.warning(
                    f"Large PDF file ({pdf_file.stat().st_size / 1024 / 1024:.1f}MB): {pdf_path}"
                )
            ...
            text_context = self._extract_text_context(
                pdf_path, acrofields, context_radius
            )

            # Step 3: Combine data and format output
            logger.info("Combining field and context data...")
            combined_data = self._combine_field_and_context_data(
                acrofields, text_context
            )
            ...
            logger.info(
                "Skipping automatic RadioGroup generation - using PDF field types..."
            )
            ...
            return {
                ...
                "csv_export_success": (
                    csv_export_success if output_format.lower() == "csv" else None
                ),
            }
            ...
            logger.error(
                f"Unexpected error extracting fields from {pdf_path}: {str(e)}"
            )
        ...
        try:
            with open(pdf_path, "rb") as file:
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to read PDF with PyPDF2: {pdf_path} - {str(e)}"
                    )
                ...
                if pdf_reader.is_encrypted:
                    logger.warning(
                        f"PDF is encrypted, attempting to decrypt: {pdf_path}"
                    )
                    try:
                        pdf_reader.decrypt("")  # Try empty password
                    except Exception as e:
                        raise ValueError(
                            f"Cannot decrypt encrypted PDF: {pdf_path} - {str(e)}"
                        )
                ...
                try:
                    ...
                    logger.warning(
                        f"Error accessing form fields, PDF may not have AcroForm: {pdf_path} - {str(e)}"
                    )
                    return []
                    ...
                    try:
                        ...
                        if field_data:
                            ...
                            if field_data.get("type") == "RadioGroup":
                                child_fields = self._extract_radiogroup_children(
                ...
                annotation_data = self._extract_annotations_with_coordinates(
                ...
                if hasattr(field_obj, "field_type"):
                    ...
                    if "/Tx" in field_type:
                        ...
                        if field_name.endswith("--group"):
                            ...
                            field_data["type"] = self._determine_button_type(
    ...
    def _extract_radiogroup_children(
        self, group_name: str, group_obj: Any
    ) -> List[Dict]:
        ...
        try:
            ...
            group_dict = (
                group_obj.get_object()
                if hasattr(group_obj, "get_object")
                else group_obj
            )
            ...
            if isinstance(group_dict, dict) and "/Kids" in group_dict:
                ...
                for i, kid in enumerate(kids):
                    try:
                        ...
                        logger.debug(
                            f"Error processing child {i} of {group_name}: {str(e)}"
                        )
                        continue

        except Exception as e:
            logger.error(
                f"Error extracting children from RadioGroup {group_name}: {str(e)}"
            )

        logger.info(
            f"Extracted {len(child_fields)} child RadioButtons from {group_name}"
        )
        return child_fields

    def _extract_annotations_with_coordinates(
        self, pdf_reader, form_fields: Dict
    ) -> Dict[str, Dict]:
        ...
        try:
            for page_num in range(len(pdf_reader.pages)):
                ...
                logger.debug(
                    f"Processing {len(annotations)} annotations on page {page_num}"
                )
                ...
                for annot_ref in annotations:
                    try:
                        ...
                        if (
                            "/Subtype" not in annot
                            or annot.get("/Subtype") != "/Widget"
                        ):
                            continue

                        # Extract field data with coordinates
                        field_info = self._process_field_annotation_enhanced(
                            annot, page_num, form_fields
                        )
                        ...
                        logger.debug(
                            f"Error processing annotation on page {page_num}: {str(e)}"
                        )
                        continue
    ...
    def _process_field_annotation_enhanced(
        self, annot: Dict, page_num: int, form_fields: Dict
    ) -> Optional[Dict]:
        ...
        try:
            ...
            field_info["type"] = self._determine_field_type_enhanced(
                annot, form_fields.get(field_name)
            )
    ...
    def _merge_annotation_data(
        self, acrofields: List[Dict], annotation_data: Dict[str, Dict]
    ) -> List[Dict]:
        ...
        for field in acrofields:
            ...
            if field_name in annotation_data:
                ...
                enhanced_field["height"] = annot_data.get(
                ...
                # Update flags
                enhanced_field["flags"] = annot_data.get(
    ...
    def _process_field_annotation(
        ...
        try:
            ...
            # Get field type
            field_data["type"] = self._determine_field_type(
                annot, form_fields.get(field_name)
            )
    ...
    def _extract_text_context(
        self, pdf_path: str, fields: List[Dict], radius: int
    ) -> Dict:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for field in fields:
                    ...
                    if x == 0 and y == 0 and width == 0 and height == 0:
                        logger.debug(
                            f"Skipping context extraction for {field_name} - no coordinates"
                        )
                        continue
    ...
    def _extract_field_context_regions(
        self, page, x: float, y: float, width: float, height: float, radius: int
    ) -> Dict[str, str]:
        ...
        regions = {
            "surrounding_text": self._get_text_in_bbox(
                page, x - radius, y - radius, x + width + radius, y + height + radius
            ),
            "text_above": self._get_text_in_bbox(
                page, x - radius, y + height, x + width + radius, y + height + radius
            ),
            "text_below": self._get_text_in_bbox(
                page, x - radius, y - radius, x + width + radius, y
            ),
            "text_left": self._get_text_in_bbox(page, x - radius, y, x, y + height),
            "text_right": self._get_text_in_bbox(
                page, x + width, y, x + width + radius, y + height
            ),
        }
    ...
    def _get_text_in_bbox(
        ...
        try:
            ...
            logger.debug(
                f"Error extracting text from bbox ({x0}, {y0}, {x1}, {y1}): {str(e)}"
            )
            return ""
    ...
    def _infer_bem_category(self, field_name: str, field_data: Dict) -> str:
        ...
        if "_" in field_name and any(
            block in field_name_lower
            for block in ["personal-information", "contingent-benficiary", "sign-here"]
        ):
        ...
        if any(
            term in field_name_lower
            for term in ["name", "first", "last", "middle", "mi"]
        ):
            ...
            term in field_name_lower
            for term in ["beneficiary", "contingent", "primary"]
        ):
            return "contingent-benficiary"
        elif any(
            term in field_name_lower for term in ["employer", "company", "organization"]
        ):
            return "employer-information"
        elif any(
            term in field_name_lower for term in ["tax", "withholding", "allowance"]
        ):
            return "tax-information"
        else:
            return "general-information"

    def _combine_field_and_context_data(
        self, acrofields: List[Dict], text_context: Dict
    ) -> List[Dict]:
        ...
        for field in acrofields:
            ...
            if field.get("type") == "RadioGroup":
                logger.debug(
                    f"Processing RadioGroup: {field_name}, type after copy: {combined_field.get('type')}"
                )
            ...
            if original_type != new_type:
                logger.debug(
                    f"Field type changed for {field_name}: {original_type} -> {new_type}"
                )

            combined_data.append(combined_field)

        logger.info(
            f"Successfully combined {len(combined_data)} fields with context data"
        )
    ...
    def _process_field_relationships(self, field_data: List[Dict]) -> List[Dict]:
        ...
        for field in field_data:
            ...
            processed_field["category_fields_count"] = len(
                categories.get(field.get("bem_category", "general-information"), [])
            )
            ...
            processed_field["related_fields"] = self._find_related_fields(
                field, field_data
            )
            processed_field["field_hierarchy_level"] = self._determine_hierarchy_level(
                field
            )

            # Add field ordering within category
            category_fields = categories.get(
                field.get("bem_category", "general-information"), []
            )
            processed_field["category_order"] = self._get_field_order_in_category(
    ...
    def _detect_radio_button_groups(self, field_data: List[Dict]) -> List[Dict]:
        ...
        for field in field_data:
            ...
            if field.get("type") == "RadioButton" or self._is_radio_button_field(
                field_name
            ):
                ...
                for other_field in field_data:
                    ...
                    if (
                        self._get_radio_button_base_name(other_name) == base_name
                        and other_field["id"] not in processed_fields
                    ):
    ...
    def _find_field_radio_group(
        self, field: Dict, radio_groups: List[Dict]
    ) -> Optional[Dict]:
    ...
    def _get_field_order_in_category(
        self, field: Dict, category_fields: List[Dict]
    ) -> int:
    ...
    def _generate_radio_group_containers(self, field_data: List[Dict]) -> List[Dict]:
        ...
        for field in field_data:
            ...
            if field_type in [
                "CheckBox",
                "RadioButton",
            ] or self._is_radio_button_field_by_pattern(field_name):
        ...
        for field in enhanced_fields:
            ...
            if group_base and group_base not in created_groups:
                ...
                if (
                    len(group_info["fields"]) > 1
                ):  # Only create if multiple radio buttons
    ...
    def _create_radio_group_container(self, group_info: Dict) -> Dict:
        ...
        if group_info["fields"]:
            ...
            radio_group["bem_category"] = first_field.get(
    ...
    def _create_csv_compatible_field(self, field: Dict) -> Dict:
        ...
        csv_field = {
            ...
            "Custom": "false",
            "Show group label": (
                "true" if field.get("type") == "RadioGroup" else "false"
            ),
    ...
    def export_to_csv(
        self, field_data: List[Dict], output_path: str, form_id: str = "52471"
    ) -> bool:
        ...
        try:
            ...
            with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
                ...
                # Write data rows directly (field_data is already in CSV format from _enhance_output_structure)
                for field in field_data:
                    # Convert field dict to row format matching csv_headers order
                    row = [
                        field.get("ID", ""),
                        field.get("Created at", ""),
                        field.get("Updated at", ""),
                        field.get("Label", ""),
                        ...
                        field.get("Type", ""),
                        field.get("Parent ID", ""),
                        field.get("Delete Parent ID", ""),
                        field.get("Acrofieldlabel", ""),
                        field.get("Section ID", ""),
                        field.get("Excluded", ""),
                        field.get("Partial label", ""),
                        field.get("Custom", ""),
                        field.get("Show group label", ""),
                        field.get("Height", ""),
                        field.get("Page", ""),
                        field.get("Width", ""),
                        field.get("X", ""),
                        field.get("Y", ""),
                        field.get("Unified field ID", ""),
                        field.get("Delete", ""),
                        field.get("Hidden", ""),
                        field.get("Toggle description", ""),
                    ]
                    writer.writerow(row)

            logger.info(f"Successfully exported {len(field_data)} rows to {output_path}")
            return True
    ...
    def _convert_fields_to_csv_rows(
        self, field_data: List[Dict], form_id: str
    ) -> List[List]:
        ...
        for order, field in enumerate(sorted_fields, 1):
            ...
            csv_row = [
                ...
                section_id_map.get(
                    field.get("bem_category", "general-information"), 8261
                ),  # Section ID
                "false",  # Excluded
        ...
        if field.get("type") == "RadioButton":
            ...
            if parent_group:
                ...
                for i, other_field in enumerate(all_fields):
                    if (
                        other_field.get("type") == "RadioGroup"
                        and other_field.get("name") == parent_group
                    ):
    ...
    def _generate_section_ids(self, field_data: List[Dict]) -> Dict[str, int]:
        ...
        categories = set(
            field.get("bem_category", "general-information") for field in field_data
        )
