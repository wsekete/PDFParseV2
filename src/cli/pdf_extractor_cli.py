...
@click.option(
    ...
    "-o",
    type=click.Path(),
    help="Output file path (auto-detect format by extension)",
)
@click.option(
    "--format",
    "-f",
...
@click.option(
    "--context-radius", "-r", default=50, help="Text context radius in pixels"
)
...
def extract(pdf_path: str, output: str, format: str, context_radius: int, pretty: bool):
    ...
    try:
        ...
        result = extractor.extract_fields(
            pdf_path=pdf_path, context_radius=context_radius, output_format=format
        )
        ...
        # Success message
        click.echo(
            f"✅ Extracted {result['field_count']} fields from {result['pages_processed']} pages"
        )
...
@click.option(
    "--pattern", "-p", default="*.pdf", help='File pattern to match (e.g., "*.pdf")'
)
@click.option(
    "--context-radius", "-r", default=50, help="Text context radius in pixels"
)
@click.option(
    "--parallel", is_flag=True, help="Process files in parallel (experimental)"
)
@click.option(
    "--continue-on-error",
    is_flag=True,
...
def batch(
    ...
    if not pdf_files:
        click.echo(
            f"❌ No PDF files found matching pattern '{pattern}' in {input_dir}",
            err=True,
        )
        sys.exit(1)
        ...
        for pdf_file in pdf_list:
            try:
                ...
                if result["success"]:
                    ...
                    if format == "csv":
                        success = extractor.export_to_csv(
                            result["data"], str(output_file)
                        )
                        if not success:
                    ...
                    if not continue_on_error:
                        click.echo(
                            f"\\n❌ Processing failed for {pdf_file}: {result.get('error')}",
                            err=True,
                        )
                        sys.exit(1)
                ...
                if not continue_on_error:
                    click.echo(
                        f"\\n❌ Unexpected error processing {pdf_file}: {str(e)}",
                        err=True,
                    )
                    sys.exit(1)
    ...
    click.echo(
        f"   📄 Total fields extracted: {sum(r['field_count'] for r in results)}"
    )
        ...
        if result["success"]:
            ...
            if radio_groups:
                ...
                for group in radio_groups[:3]:  # Show first 3
                    ...
                    children = [
                        b for b in radio_buttons if b.get("Parent ID") == group_id
                    ]
                    click.echo(
                        f"      - {group.get('Api name', 'Unknown')}: {len(children)} buttons"
                    )
            ...
            click.echo(
                f"   ❌ Field extraction failed: {result.get('error', 'Unknown error')}"
            )
